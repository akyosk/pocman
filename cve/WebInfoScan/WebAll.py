...
class JsFinderScan2:
    ...
    def find_by_url_deep(self, url):
        ...
        for link in links:
            ...
            OutPrintInfo("Web-All",
                         f"从[b bright_red]{str(i)}[/b bright_red]个结果中找到[b bright_red]{str(len(temp_urls))}[/b bright_red]个URL在[b bright_red]{link}[/b bright_red]")
            for temp_url in temp_urls:
    ...
    def giveresult(self, urls, domian):
        ...
        for subdomain in subdomains:
            ...
            OutPrintInfo("Web-All", f"[b bright_red]{subdomain}[/b bright_red]")
    ...
    def xss(self, url, payload):
        ...
        try:
            ...
            if payload in response.text and response.status_code == 200:
                OutPrintInfo("Web-All", f"存在[b bright_red]XSS漏洞[/b bright_red],URL:{req_url}")
    ...
    def sql_time(self, url):
        ...
        try:
            ...
            if end - start > 10:
                OutPrintInfo("Web-All", f"存在[b bright_red]SQL-Time漏洞[/b bright_red],URL:{url}")
        except Exception as e:
            pass
    ...
    def sql_get(self, url, req_len):
        ...
        try:
            ...
            if req_len != resp_len:
                OutPrintInfo("Web-All",
                             f"[b bright_red]存在SQL-Get漏洞[b bright_red],URL:{url} 正常请求长度:[b bright_red]{req_len}[/b bright_red] | 注入请求长度:[b bright_red]{resp_len}[/b bright_red]")
        except Exception as e:
            pass
    ...
    def webDir(self, url):
        ...
        try:
            ...
            if 'attachment' in contents.lower() or "root:" in response.text:
                OutPrintInfo("Web-All", f"存在[b bright_red]目录穿越漏洞[/b bright_red],URL:{url}")
        except Exception as e:
            pass
    ...
    def check(self, urls):
        ...
        for ck in urls:
            if "=" in ck:
                ck_url.append(ck)
            if "admin" in ck:
                OutPrintInfo("Web-All", f"[b bright_red]检测到敏感路径[/b bright_red]:{ck}")
    ...
    def main(self, target):
        urllib3.disable_warnings()
        url = target[0].strip('/ ')
        self.cookie = target[1]
        depth = target[2]
        self._threads = int(target[3])

        proxy = target[4]
        self._ssl = target[5]

        req = ReqSet(proxy=proxy)
        self._proxy = req["proxy"]
