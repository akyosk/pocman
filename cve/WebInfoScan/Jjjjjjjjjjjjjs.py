...
def modeParserImplement2(target_parmses):
    global cookieConf
    global headersConf
    global threadsConf
    global modeConf
    ...
    api = None
    proxy =None
    target_url = target_parmses[0]
    if target_parmses[1]:
        cookie = "cookie="+target_parmses[1]
    header = "header="+target_parmses[2]
    if target_parmses[3]:
        api = "api="+target_parmses[3]
    thread = "thread="+str(target_parmses[4])
    if target_parmses[5]:
        proxy = "proxy="+target_parmses[5]


    #目标判断
    parseResult["target"]=target_url
    OutPrintInfo("1","直接开始爬取")
    OutPrintInfo("2","直接开始fuzz")
    OutPrintInfo("3","禁用输出响应body")
    OutPrintInfo("4","排除输入的指定api")
    OutPrintInfo("5","用户指定api根路径")
    OutPrintInfo("6","对500 401 403 进行bypass测试")
    OutPrintInfo("7","解除危险接口限制")
    OutPrintInfo("8","仅获取有效api，无后续响应获取")
    OutPrintInfo("JS","上面选项除1外部分可进行多选:2,3 | ...")
    ...
    if choose=="1":
        ...
        return parseResult
    choose_list = []
    choose_list.append(header)
    choose_list.append(thread)
    if target_parmses[3]:
        choose_list.append(api)

    if target_parmses[5]:
        choose_list.append(proxy)

    if target_parmses[1]:
        choose_list.append(cookie)

    if "2" in choose:
        choose_list.append("fuzz")
    elif "3" in choose:
        choose_list.append("nobody")
    elif "4" in choose:
        choose_list.append("noapi")
    elif "5" in choose:
        choose_list.append("api")
    elif "6" in choose:
        choose_list.append("bypass")
    elif "7" in choose:
        choose_list.append("danger")
    elif "8" in choose:
        choose_list.append("nofuzz")
    # print(choose_list)
    params=[]
    for i in range(0,len(choose_list)):
        params.append(choose_list[i])
    ...
    for i in range(0,len(choose_list)):
        ...
        if choose_list[i].split("=")[0].lower()=="cookie":
            if DEBUG and Verbose:
                print(choose_list[i])
            isCookie=True
            cookieConf=choose_list[i].split("=",1)[-1].strip()
        elif choose_list[i].split("=")[0].lower()=="header":
            if DEBUG and Verbose:
                print(choose_list[i])
            isHeader=True
            #header使用 [=] 赋值，: 分隔单个header的键值 以\n分隔不同header
            headerRaw=choose_list[i].split("=",1)[-1]
            if DEBUG and Verbose:
        ...
        #fuzz|api
        elif choose_list[i].lower()=="fuzz":
            # parseResult["mode"]+="fuzz"
            mode.append("fuzz")
            if DEBUG and Verbose:
                print(parseResult["mode"])
        elif choose_list[i].replace("api","",1).startswith("="):
            if choose_list[i].split("=")[0].lower()=="api":
                mode.append("api")
                apis=choose_list[i].split("=")[-1].split(",")
                parseResult["apis"]=apis

        elif choose_list[i].lower()=="api":
                mode.append("api")
        #noapi
        elif choose_list[i].replace("noapi","",1).startswith("="):
            if choose_list[i].split("=")[0].lower()=="noapi":
                mode.append("noapi")
                noapis=choose_list[i].split("=")[-1].split(",")
                parseResult["noapis"]=noapis
        elif choose_list[i].lower()=="noapi":
            mode.append("noapi")
        #nobody|nofuzz
        elif choose_list[i].lower()=="nobody":
            mode.append("nobody")
        elif choose_list[i].lower()=="nofuzz":
            mode.append("nofuzz")
        #danger
        elif choose_list[i].lower()=="danger":
            mode.append("danger")
            isDangerRemove=False
        #bypass
        elif choose_list[i].lower()=="bypass":
            mode.append("bypass")
            isBypassOn=True

        elif choose_list[i].replace("thread","",1).startswith("="):
            if choose_list[i].split("=")[0].lower()=="thread":
                mode.append("thread")
                threadsConf=int(choose_list[i].split("=")[-1])
                debugger(threadsConf,"thread")
        elif choose_list[i].lower()=="thread":#这里永远不会生效，在其他地方已处理
            mode.append("thread")
            threadsConf=50
        elif choose_list[i].replace("proxy","",1).startswith("="):
            if choose_list[i].split("=")[0].lower()=="proxy":
                mode.append("proxy")
                proxy=choose_list[i].split("=")[-1]
                proxyConf={"http":proxy,"https":proxy}
                debugger(proxyConf,"proxyConf")
        elif choose_list[i].lower()=="proxy":
            mode.append("proxy")
...
def main2(targetsss):
    ...
    modeparser=modeParserImplement2(targetsss)
