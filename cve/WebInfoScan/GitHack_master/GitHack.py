...
import re
import time
from cve.WebInfoScan.GitHack_master.lib.parser import parse
import ssl
...
class GitHackScan(object):

    def main(self,target):
        self.base_url = target['url'].strip("/ ")
        self.domain = urlparse.urlparse(self.base_url).netloc.replace(':', '_')
        print('[+] Download and parse index file ...')
        try:
            data = self._request_data(self.base_url + '/index')
        except Exception as e:
            print('[ERROR] index file download failed: %s' % str(e))
            return
        with open('./result/index', 'wb') as f:
            f.write(data)
        if not os.path.exists(f"./result/{self.domain}"):
            os.mkdir(f"./result/{self.domain}")
        ...
        while not self.STOP_ME:
            ...
            for i in range(3):
                try:
                    ...
                    if target_dir and not os.path.exists(f"./result/{target_dir}"):
                        os.makedirs(f"./result/{target_dir}")
                    with open(os.path.join(f"./result/{self.domain}", file_name), 'wb') as f:
                        f.write(data)
