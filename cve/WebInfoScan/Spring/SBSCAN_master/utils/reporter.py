...
class ReportGenerator:
    def __init__(self, output_folder='./result', quiet=False, pbar=None):
        self.quiet = quiet
        self.report_data = []
    ...
    def save_report_to_file(self):
        ...
        with open(f"./result/{filename}", 'a', encoding='utf-8') as file:
            json.dump(report_data_copy, file, indent=4, ensure_ascii=False)

        self.console.print(f"[cyan][+] 报告已保存到[The report was saved to]: [bold yellow]result/{filename}[/bold yellow]")
