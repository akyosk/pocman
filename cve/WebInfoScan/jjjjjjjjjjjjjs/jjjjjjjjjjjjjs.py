...
# 获取当前工作目录
current_working_directory = os.getcwd().split('/')[0:-3]
res_current_working_directory = "/".join(current_working_directory)
BaseDir=f"{res_current_working_directory}/result/"#项目输出目录
...
def removeDangerousApi(urlList):
    ...
    filename=f"{res_current_working_directory}/result/js_dangerous.txt"
...
def urlToFile(mode,origionUrl,filename):
    ...
    rawFilename=f"{res_current_working_directory}/result/.js_raw_spider.txt"
    filename=f"{res_current_working_directory}/result/.js_result.txt"
    ...
    #输出接口字典到文件
    print()
    interfaceDicc=f"{res_current_working_directory}/result/.js.txt"
    myspider=apiFuzz()
...
def singleUserInputApi(mode,origionUrl,apiPaths):
    ...
    if not urlList:
        ...
        filename=f"{res_current_working_directory}/result/.js.txt"
...
class apiFuzz:
    ...
    def spiderTaskStatusOutput(self,mode,spiderResult):
        ...
        # print(f"spider输出测试=========================")
        #输出原始响应列表至文件 .js_raw_resp.txt
        filename=f"{res_current_working_directory}/result/.js_raw_resp.txt"
    ...
    def apiFuzzForUserInputApiInAction(self,mode,origionUrl,apiPaths,apiFuzzList,anchorRespList=[],threads=100):
        ...
        if "nofuzz" not in mode:
            ...
            if DEBUG:
                ...
                filename=f"{res_current_working_directory}/result/.js_fuzz_url.txt"
...
proxyConf={}
outputConf=f"{res_current_working_directory}/result/jsresult.txt"#输出文件名/文件夹
