...
class FastadminInfoScan:
    ...
    def run(self, urls):
        try:
            ...
            if "_DATABASE" in response.text or "_ACCESSKEYID" in response.text or "_REDIS" in response.text:
                # OutPrintInfo("DocCms", '[b bright_red]存在SQL注入 ')
                OutPrintInfo("FastAdmin", f"[b bright_red]存在敏感信息 {url}")
                with open("./result/fastadminInfoVuls.txt", "a") as w:
                    w.write(f"{url}\n")
            else:
