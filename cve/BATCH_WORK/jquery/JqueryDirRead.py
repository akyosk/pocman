...
class JqueryDirReadScan:
    ...
    def run(self,url):
        try:
            response = requests.post(url, headers=self.headers, verify=self.verify, proxies=self.proexis,timeout=self.timeout)
            contents = response.headers.get('Content-Disposition', '')
            if 'attachment' in contents.lower():
                # OutPrintInfo("JQuery", f"可能存在Juqery-1.7.2任意文件读取漏洞")
                OutPrintInfo("JQuery", f"Url:{url}")
                with open("./result/jqueryDirRead.txt","a") as w:
                    w.write(f"{url}\n")
