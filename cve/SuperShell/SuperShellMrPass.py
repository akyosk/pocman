...
class SuperShellMrPass_Scan:
    ...
    def run4(self,url):
        ...
        try:
            ...
            if "success" == json_str.get("result"):
                OutPrintInfoSuc("SuperShell", f"目标存在默认密钥漏洞:{base_url} | {username}/{username}123")
                if self.batch:
                    OutPutFile("supershell_moren_passwd.txt",f"目标存在默认密钥漏洞: {base_url} | {username}/{username}123")
    ...
    def run6(self,url):
        ...
        try:
            ...
            if "success" == json_str.get("result"):
                OutPrintInfoSuc("SuperShell", f"目标存在默认密钥漏洞:{base_url} | {username}/admin123")
                if self.batch:
                    OutPutFile("supershell_moren_passwd.txt",f"目标存在默认密钥漏洞: {base_url} | {username}/admin123")
    ...
    def run7(self,url):
        ...
        if username == None:
            username = "admin"
        data = {"username":username,"password":username+"@888"}
        try:
            response = requests.post(base_url,headers=headers,verify=self.ssl,proxies=self.proxy,timeout=self.timeout,json=data)
            json_str = json.loads(response.text)

            if "success" == json_str.get("result"):
                OutPrintInfoSuc("SuperShell", f"目标存在默认密钥漏洞:{base_url} | {username}/{username}@888")
                if self.batch:
                    OutPutFile("supershell_moren_passwd.txt",f"目标存在默认密钥漏洞: {base_url} | {username}/{username}@888")
