...
class Cve_2023_33246:
    ...
    def detect_version(self, ip, port, data):
        parsed_data = data
        ...
        for version in broker_version_descs:
            ...
            if (version > 500 and version < 511) or (version > 400 and version < 496):
                OutPrintInfo("RocketMQ",'{ip}:{port} is an unauthorized RocketMQ dashboard and Vulnerable to CVE-2023-33246 RocketMQ RCE, broker addr: {broker_addrs}'.format(ip=ip, port=port, version_str=version_str, broker_addrs=broker_addrs))
            else:
                OutPrintInfo("RocketMQ","{ip}:{port} NOT Vulnerable to CVE-2023-33246 RocketMQ RCE".format(ip=ip, port=port))

    def is_rocketmq_dashboard(self, ip, port):
        try:
            ...
            if resp.status_code == 200 and 'json' in resp.headers['Content-Type']:
                data = resp.json()
                self.detect_version(ip, port, data)
            else:
                return False
    ...
    def send_data_to_broker(self, ip, port):
        resp = None
        ...
        try:
            ...
            resp = s.recv(1024)
            if not resp:
                OutPrintInfo("RocketMQ",f"{ip} is not reachable, revice nothing from {port}.")
            # Close the socket
            s.close()
        except Exception as e:
            OutPrintInfo("RocketMQ",f"{ip}:{port} is not reachable: {e}")
        return resp

    def check_vulnerability(self, target):
        """Check if the target is vulnerable"""
        ip, port = target.get('ip'), int(target.get('port'))
        if not self.is_rocketmq_dashboard(ip, port):
            resp = self.send_data_to_broker(ip, port)
            broker_addr = []
            data = resp[8:].decode()
            if data.startswith('{'):
                if '}{' in data:
                ...
                if target_version.startswith('V4.9.'):
                    ...
                    if minior_version < 6:
                        OutPrintInfo("RocketMQ",f"[b bright_red]{ip}:{port}[/b bright_red] Vulnerable to CVE-2023-33246 RocketMQ RCE, version: {target_version}, brokers: {', '.join(broker_addr)}")
                    else:
                        OutPrintInfo("RocketMQ",f"{ip}:{port} Not vulnerable to CVE-2023-33246 RocketMQ RCE, version: {target_version} >= V4.9.6")
                elif target_version.startswith('V5.1.'):
                    minior_version = int(target_version.split('.')[2])
                    if minior_version < 1:
                        OutPrintInfo("RocketMQ",f"[b bright_red]{ip}:{port}[/b bright_red] Vulnerable to CVE-2023-33246 RocketMQ RCE, version: {target_version}, brokers: {', '.join(broker_addr)}")
                    else:
                        OutPrintInfo("RocketMQ",f"{ip}:{port} Not Vulnerable to CVE-2023-33246 RocketMQ RCE, version: {target_version} >= V5.1.1")
                else:
                    OutPrintInfo("RocketMQ",f"{ip}:{port} Not vulnerable to CVE-2023-33246 RocketMQ RCE, version: {target_version}")
            else:
                OutPrintInfo("RocketMQ",f"{ip}:{port} Not vulnerable to CVE-2023-33246 RocketMQ RCE")

    def main(self, target):

        ip = target["ip"]
        if '://' in ip:
            OutPrintInfo("RocketMQ",'仅支持IP......')
            return
        port = target["port"]
        target_list = []
        ...
        if not target_list:
            OutPrintInfo("RocketMQ","Please provide at least one IP address")
            return

        with ThreadPoolExecutor() as executor:
            for target in target_list:
                executor.submit(self.check_vulnerability, target)
