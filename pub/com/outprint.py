...
def PocPrint(version,ip,pocs,at_pocs,yaml_pocs,nums):
    if rich_flag:
        ...
        rp(f":smile:[b yellow]共收录内置POC [b bright_red]{pocs}[/b bright_red] 个[/b yellow]")
        rp(
            f":+1:[b cyan]共收录Batch-POC [b bright_red]{at_pocs}[/b bright_red] 个[/b cyan]")
        rp(
            f":boom:[b magenta]共收录Yaml-POC [b bright_red]{yaml_pocs}[/b bright_red] 个[/b magenta]")
        if nums:
            rp(
                f":gun:[b bright_magenta]共 [b bright_red]{nums}[/b bright_red] 个Yaml-POC加载出错 [/b bright_magenta]")
    else:
        ...
        rp(f"[b yellow]共收录内置POC [b bright_red]{pocs}[/b bright_red] 个[/b yellow]")
        rp(
            f"[b cyan]共收录Batch-POC [b bright_red]{at_pocs}[/b bright_red] 个[/b cyan]")
        rp(
            f"[b magenta]共收录Yaml-POC [b bright_red]{yaml_pocs}[/b bright_red] 个[/b magenta]")
        if nums:
            rp(
                f":gun:[b bright_magenta]共 [b bright_red]{nums}[/b bright_red] 个Yaml-POC加载出错 [/b bright_magenta]")
# Attack-Post输出格式
