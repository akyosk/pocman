...
from libs.outprint import OutPrintInfo, OutPrintInfoErr,OutPrintInfoSuc
...
class AT_RUN_WORK:
    ...
    def crawer(self,url,html_code=None):
        try:
            if not html_code:
                html_code = requests.get(url, headers=self.headers, timeout=self.timeout, verify=self._ssl, proxies=self._proxy).text
            list_r = []
            # print(html_code)
    ...
    def ck(self,url):
        try:
            req = requests.get(url,headers=self.headers, timeout=self.timeout, verify=self._ssl, proxies=self._proxy)
