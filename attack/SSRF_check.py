    ...
    def ssrf_check(self, url):
        try:
            ...
            if "root:x" in req.text or "16-bit" in req.text:
                return True, self.__base_url, "SSRF VULN", req.request.headers
        except Exception:
            pass

    def ssrf_check_post(self, url):
        ...
        try:
            ...
            if "root:x" in req.text or "16-bit" in req.text:
                return True, url, "SSRF POST Inject", req.request.headers,req.request.body
        except Exception:
            pass

    def main(self, target, attack_url_list, progress, __craw_post_url_list):
        self.__attack_url_list = attack_tihuan_url_canshu_work(attack_url_list, "ssrf")
        if target['proxy']:
            if "://" in target['proxy']:
        ...
        tasks = progress.add_task("[b blue]SSRF检测...", total=len(self.__attack_url_list))
        with ThreadPoolExecutor(self.__threads) as pool:
            futures = [pool.submit(self.ssrf_check, base_url) for base_url in self.__attack_url_list]
        ...
        post_att_url_list = post_attack_tihuan_url_canshu_work(self.__base_url, __craw_post_url_list, "ssrf")
        if post_att_url_list:
            tasks = progress.add_task("[b blue]SSRF POST检测...", total=len(post_att_url_list))
            with ThreadPoolExecutor(self.__threads) as pool:
                futures = [pool.submit(self.ssrf_check_post, base_url) for base_url in post_att_url_list]
                for future in as_completed(futures):
                    result = future.result()
                    if result is not None:
